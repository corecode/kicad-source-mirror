 ...
 */
static void calcLogDecadeTicks( std::vector<double>& aTickValues, std::vector<mpScaleBase::TICK_LABEL>& aTickLabels,
                                double aMinVis, double aMaxVis, int aClampMin = INT_MIN, int aClampMax = INT_MAX )
{
    aTickValues.clear();
    aTickLabels.clear();
...
{
public:
    PDN_LOG_SCALE( int aAlign, const wxString aUnit, int aClampMin = INT_MIN, int aClampMax = INT_MAX ) :
            BASE( wxEmptyString, aAlign, false ),
            m_unit( aUnit ),
            m_clampMin( aClampMin ),
    ...
    const std::vector<typename BASE::TICK_LABEL>& GetTickLabels() const { return this->m_tickLabels; }
...
protected:
    void recalculateTicks( wxDC& dc, mpWindow& w ) override
    {
        ...
        calcLogDecadeTicks( this->m_tickValues, this->m_tickLabels, minVvis, maxVvis, m_clampMin, m_clampMax );
...
public:
    PDN_XSCALE() :
            PDN_LOG_SCALE( mpALIGN_BORDER_BOTTOM, wxS( "Hz" ) )
    {
    }
};
...
public:
    PDN_YSCALE() :
            PDN_LOG_SCALE( mpALIGN_BORDER_LEFT, wxS( "\u03A9" ), LOG_Z_MIN, LOG_Z_MAX )
    {
    }
};
            ...
            if( m_transferClampedLeft )
            {
                ...
                if( px >= w.GetMarginLeft() && px <= w.GetScrX() - w.GetMarginRight() )
                {
                    wxPoint tri[3] = { wxPoint( px - triSize, plotBottom - triSize * 2 ),
                                       wxPoint( px + triSize, plotBottom - triSize * 2 ), wxPoint( px, plotBottom ) };
...
private:
    ...
    {
        const std::vector<double>& ticks = aHorizontal ? m_xAxis->GetTickValues() : m_yAxis->GetTickValues();
        ...
        bool halfMinors = ( ( endPx - startPx ) / ( ticks.size() / 9 + 1 ) ) < MINOR_REDUCE_THRESHOLD;
...
PDN_PANEL::PDN_PANEL( SCH_EDIT_FRAME* aParent ) :
        ...
        m_referenceTrace( nullptr ),
        m_transferTrace( nullptr ),
        m_xAxis( nullptr ),
        m_yAxis( nullptr ),
        m_labelOverlay( nullptr ),
        m_warningIcon( nullptr )
{
    ...
    m_plotWindow->Bind( wxEVT_LEFT_DOWN,
                        []( wxMouseEvent& )
                        {
                        } );
    m_plotWindow->Bind( wxEVT_LEFT_UP,
                        []( wxMouseEvent& )
                        {
                        } );
    m_plotWindow->Bind( wxEVT_RIGHT_DOWN, &PDN_PANEL::onPlotRightClick, this );
    m_plotWindow->Bind( wxEVT_SIZE,
...
void PDN_PANEL::updateWarningIndicator()
{
    ...
    wxSize iconSize = m_warningIcon->GetSize();
    wxSize plotSize = m_plotWindow->GetClientSize();
    m_warningIcon->SetPosition( wxPoint( plotSize.x - iconSize.x - 4, plotSize.y - iconSize.y - 4 ) );
}
...
{
    wxString projectPath = m_frame->Schematic().Project().GetProjectPath();
...
void PDN_PANEL::OnSchSelectionChanged()
{
    if( !IsShownOnScreen() )
        return;
...
void PDN_PANEL::onPlotRightClick( wxMouseEvent& aEvent )
{
    ...
    if( !fbEntries.empty() )
    {
        ...
        for( size_t i = 0; i < fbEntries.size(); i++ )
        {
            ...
            if( !displayMpn.IsEmpty() )
            {
                label = wxString::Format( _( "%s — download %s" ), fbEntries[i].refdes, displayMpn );
            }
            else
            {
    ...
    wxMenuItem* transferItem = menu.AppendCheckItem( showTransferId, _( "Show Transfer Impedance" ) );
    ...
    menu.Bind(
            ...
            [this, &visibleKeys, &fbEntries, fbMenuBase, showTransferId, storeRefId, clearRefId]( wxCommandEvent& evt )
            {
                int id = evt.GetId();
                ...
                if( netIdx >= 0 && netIdx < static_cast<int>( visibleKeys.size() ) )
                {
                    ...
                    m_frame->GetToolManager()->RunAction( SCH_ACTIONS::updateNetHighlighting );
                }
                ...
                {
                    onGetSpiceModel( fbEntries[fbIdx].mpn, fbEntries[fbIdx].refdes, fbEntries[fbIdx].value );
                }
                ...
                if( id == storeRefId )
                {
                    ...
                    {
                        m_refLabel =
                                wxString::Format( wxS( "%s / %s" ), netIt->second.supplyRail, netIt->second.refRail );
                    }
                    ...
                    {
                        logFreqs[i] = ( m_refFrequencies[i] > 0.0 ) ? std::log10( m_refFrequencies[i] ) : 0.0;
                    }

                    for( size_t i = 0; i < m_refImpedance.size(); i++ )
                    {
                        logZ[i] = ( m_refImpedance[i] > 0.0 ) ? std::log10( m_refImpedance[i] ) : -12.0;
                    }

                    m_referenceTrace->SetData( logFreqs, logZ );
                    m_referenceTrace->SetVisible( true );
                    m_labelOverlay->SetReferenceLabel( wxString::Format( _( "ref: %s" ), m_refLabel ) );
                    m_plotWindow->UpdateAll();
                }
...
void PDN_PANEL::onGetSpiceModel( const wxString& aMpn, const wxString& aRefdes, const wxString& aValue )
{
    if( !m_downloader )
        return;
    ...
    if( mpn.IsEmpty() || !m_downloader->HasSource( mpn ) )
    {
        ...
        wxTextEntryDialog dlg( this, wxString::Format( _( "Enter manufacturer part number for %s:" ), aRefdes ),
    ...
    for( size_t i = 0; i < references.GetCount(); i++ )
    {
        ...
        SCH_FIELD* libField = symbol->FindFieldCaseInsensitive( wxT( "Sim.Library" ) );

        if( !libField )
        {
            SCH_FIELD newField( symbol, FIELD_T::USER, wxT( "Sim.Library" ) );
        ...
        SCH_FIELD* nameField = symbol->FindFieldCaseInsensitive( wxT( "Sim.Name" ) );

        if( !nameField )
        {
            SCH_FIELD newField( symbol, FIELD_T::USER, wxT( "Sim.Name" ) );
...
void PDN_PANEL::onSchematicChanged( wxCommandEvent& aEvent )
{
    ...
    // Re-register as listener — the SCHEMATIC object is replaced on file open
    m_frame->Schematic().AddListener( this );
...
void PDN_PANEL::runAnalysisAndPlot( const wxString& aNetworkKey )
{
    ...
    m_labelOverlay->SetNetworkLabel( wxString::Format( wxS( "%s / %s" ), network.supplyRail, network.refRail ) );
...
void PDN_PANEL::plotImpedance( const std::vector<double>& aFreqs, const std::vector<double>& aImpedance )
{
    ...
    m_labelOverlay->SetTransferClampIndicators( clampedLeft, clampLeftX, clampedRight, clampRightX );
