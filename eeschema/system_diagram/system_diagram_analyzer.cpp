...
bool SYSTEM_DIAGRAM_ANALYZER::Analyze()
{
    ...
    m_data.m_components.erase( std::remove_if( m_data.m_components.begin(), m_data.m_components.end(),
...
void SYSTEM_DIAGRAM_ANALYZER::buildBusGraph()
{
    ...
    // Collect all bus aliases from the schematic
    ...
    for( auto& [aliasName, alias] : aliases )
    {
        ...
        if( connectedComponents.size() >= 2 )
        {
            ...
            bus->m_connectedComponents.assign( connectedComponents.begin(), connectedComponents.end() );
...
void SYSTEM_DIAGRAM_ANALYZER::findMarkedSignals()
{
    ...
    const std::map<wxString, std::set<wxString>>& assignments = netSettings->GetNetclassLabelAssignments();
    ...
    for( const wxString& netName : markedNets )
    {
        ...
        if( connectedComponents.size() >= 2 )
        {
            ...
            sig->m_connectedComponents.assign( connectedComponents.begin(), connectedComponents.end() );
...
void SYSTEM_DIAGRAM_ANALYZER::buildPowerTree()
{
    ...
    for( auto& [key, node] : allNodes )
    {
        ...
        if( inputPinIt != refToInputPin.end() )
        {
            ...
            {
                if( rp.first == node->m_reference && rp.second.IsSameAs( inputPinIt->second, false ) )
                {
    ...
    for( auto& [key, node] : allNodes )
    {
        ...
        for( const wxString& loadRef : netIt->second )
        {
            ...
            if( outIt != refToPowerOutNets.end() && outIt->second.count( node->m_netName ) )
            {
                continue;
            }
...
double SYSTEM_DIAGRAM_ANALYZER::parseCurrent( const wxString& aValue )
{
    ...
    if( suffix.IsEmpty() || suffix.IsSameAs( wxT( "A" ), false ) )
        ...
        multiplier = 1e-3;
    else if( suffix.IsSameAs( wxT( "uA" ), false ) || suffix.StartsWith( wxS( "\u00B5" ) ) // µA (micro sign)
...
void SYSTEM_DIAGRAM_ANALYZER::bubbleUpCurrents()
{
    ...
    bubbleUp = [&]( SD_POWER_NODE* node )
    {
        ...
        for( const auto& [mode, I_out] : node->m_currentByMode )
        {
            ...
            if( node->m_converterType == SD_POWER_NODE::CONV_SMPS && node->m_voltage > 0.0
