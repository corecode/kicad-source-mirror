 ...
 */
static VRM_PARAMS readVrmParams( SCH_SYMBOL* aSymbol, const SCH_SHEET_PATH& aSheet,
                                 const wxString& aNetworkKey,
                                 std::function<void( const wxString&, const wxString& )> aWarnFn )
{
    ...
    for( const SCH_FIELD& field : aSymbol->GetFields() )
    {
        ...
        if( name == wxT( "Pwr.Zout" ) )
        {
            ...
            {
                aWarnFn( aNetworkKey,
                         wxString::Format( _( "%s: unable to parse Pwr.Zout '%s', using default." ),
        ...
        else if( name == wxT( "Pwr.BW" ) )
        {
            ...
            {
                aWarnFn( aNetworkKey,
                         wxString::Format( _( "%s: unable to parse Pwr.BW '%s', using default." ),
                                           aSymbol->GetRef( &aSheet, false ), val ) );
            }
        }
    }
...
PDN_ANALYZER::PDN_ANALYZER() : m_schematic( nullptr ), m_downloader( nullptr )
{
}
        ...
        for( SCH_ITEM* item : sheet.LastScreen()->Items() )
        {
            ...
            for( auto& [key, network] : m_networks )
            {
                if( network.vrm.has_value() )
                {
                    ...
                    {
                        addWarning( key,
                                    wxString::Format( _( "Multiple regulators on '%s': using %s, "
                                                         "ignoring %s." ),
                                                      network.supplyRail, network.vrm->refdes,
                                                      refStr ) );
                    }
        ...
        for( size_t i = 0; i < references.GetCount(); i++ )
        {
            ...
            if( elem.type == PDN_SERIES_TYPE::RESISTOR )
            {
                ...
                try
                {
                    ...
                    {
                        wxString w =
                                wxString::Format( _( "Skipping series R %s — invalid value '%s'." ),
                                                  refStr, valueStr );
                ...
                catch( ... )
                {
                    wxString w = wxString::Format(
                            _( "Skipping series R %s — unable to parse value '%s'." ), refStr,
                            valueStr );
            ...
            else
            {
                ...
                for( const SCH_FIELD& field : symbol->GetFields() )
                {
                    ...
                    if( elem.mpn.IsEmpty() )
                    {
                        ...
                        if( nameUpper == wxT( "MPN" )
                            || nameUpper == wxT( "MANUFACTURER PART NUMBER" )
                ...
                if( ( elem.spiceModel.IsEmpty() || elem.spiceLibFile.IsEmpty() )
                    && !elem.mpn.IsEmpty() && m_downloader )
                {
                ...
                {
                    wxString w =
                            wxString::Format( _( "Ferrite bead %s (%s — %s) has no SPICE model "
    ...
    for( auto& [key, network] : m_networks )
    {
        ...
        for( const VRM_CANDIDATE& cand : candidates )
        {
            ...
            for( const wxString& fbNet : cand.signalNets )
            {
                ...
                bool reachesSupply =
                        reachesThroughResistors( fbNet, network.supplyRail, network.refRail, 2 );
                bool reachesRef =
                        reachesThroughResistors( fbNet, network.refRail, network.supplyRail, 2 );
...
std::vector<wxString> PDN_ANALYZER::GetCoupledNetworkKeys( const wxString& aNetworkKey ) const
{
    ...
    for( const auto& [key, network] : m_networks )
    {
        ...
        if( railsConnected( primarySupply, network.supplyRail )
            && railsConnected( primaryRef, network.refRail ) )
        {
                                          ...
                                          int aPointsPerDecade )
{
    ...
    for( const auto& [sheetKey, caps] : sheetGroups )
    {
        ...
        wxString   sheetLabel =
                sheet ? sheet->GetName() : wxString::Format( _( "Group %d" ), groupIdx );
...
wxString PDN_ANALYZER::BuildSpiceNetlist( const wxString&       aNetworkKey,
                                          const SCH_SHEET_PATH& aObservationSheet,
                                          double aTraceInductance, double aTraceResistance,
                                          double aStartFreq, double aEndFreq, int aPointsPerDecade )
{
    m_probeNodes.clear();
    ...
    {
        return BuildSpiceNetlist( primaryIt->second, aObservationSheet, aTraceInductance,
                                  aTraceResistance, aStartFreq, aEndFreq, aPointsPerDecade );
    }
    ...
    for( const wxString& netKey : coupledKeys )
    {
        ...
        if( isPrimary )
        {
            ...
            if( localIt != sheetGroups.end() )
            {
                for( const PDN_CAPACITOR* cap : localIt->second )
                {
                    netlist += wxString::Format( wxS( "X%d %s %s CAP c=%g esr=%g esl=%g\n" ), idx,
                                                 supplyNode, refNode, cap->capacitance, cap->esr,
                                                 cap->esl );
                    idx++;
                }
            }
            ...
            for( const auto& [sheetKey, caps] : sheetGroups )
            {
                ...
                {
                    netlist += wxString::Format( wxS( "X%d %s %s CAP c=%g esr=%g esl=%g\n" ), idx,
                                                 nodeR, refNode, cap->capacitance, cap->esr,
                                                 cap->esl );
                    idx++;
                }
        ...
        else
        {
            ...
            {
                netlist +=
                        wxString::Format( wxS( "X%d %s %s CAP c=%g esr=%g esl=%g\n" ), idx,
                                          supplyNode, refNode, cap.capacitance, cap.esr, cap.esl );
                idx++;
            }
        }
        ...
        if( network.vrm.has_value() )
        {
            ...
            if( isPrimary && vrm.sheetPath.PathAsString() != obsKey )
            {
                ...
                netlist += wxString::Format( wxS( "\n* VRM trace (%s, remote)\n" ), vrm.refdes );
                netlist += wxString::Format( wxS( "L_s%d %s %s %g\n" ), groupIdx, supplyNode,
                                             vrmNodeL, aTraceInductance );
                netlist += wxString::Format( wxS( "R_s%d %s %s %g\n" ), groupIdx, vrmNodeL,
                                             vrmNodeR, aTraceResistance );
                vrmConn = vrmNodeR;

                m_probeNodes.push_back(
                        { wxString::Format( _( "VRM (%s)" ), vrm.refdes ), vrmNodeR } );
        ...
        if( !isPrimary )
        {
            ...
            for( const PDN_SERIES_ELEMENT& elem : m_seriesElements )
            {
                if( ( elem.railA == network.supplyRail || elem.railB == network.supplyRail )
                    && ( elem.railA == primaryNetwork.supplyRail
                         || elem.railB == primaryNetwork.supplyRail ) )
                {
                    label = wxString::Format( wxS( "%s (via %s)" ), network.supplyRail,
                                              elem.refdes );
                    break;
                }
            }
    ...
    for( const PDN_SERIES_ELEMENT& elem : m_seriesElements )
    {
        ...
        if( elem.type == PDN_SERIES_TYPE::RESISTOR )
        {
            ...
            netlist += wxString::Format( wxS( "R_se%d %s %s %g\n" ), seIdx, itA->second,
                                         itB->second, elem.resistance );
        }
        else if( !elem.spiceModel.IsEmpty()
                 && ( !elem.spiceLibFile.IsEmpty() || !elem.spiceText.IsEmpty() ) )
        {
            netlist += wxString::Format( wxS( "\n* Ferrite bead: %s (%s <-> %s)\n" ), elem.refdes,
                                         elem.railA, elem.railB );
            netlist += wxString::Format( wxS( "X_se%d %s %s %s\n" ), seIdx, itA->second,
                                         itB->second, elem.spiceModel );
        }
        else
        {
            ...
            netlist +=
                    wxString::Format( wxS( "R_se%d %s %s 1k\n" ), seIdx, itA->second, itB->second );
        }
                                 ...
                                 double aTraceInductance, double aTraceResistance )
{
    ...
    // Temporarily suppress the reporter — the shared ngspice singleton may have
    // one set by the SPICE simulator frame, and forwarding thousands of output
    // lines through it during our AC sweep is very slow.
...
bool PDN_ANALYZER::RunAnalysis( const wxString&       aNetworkKey,
                                const SCH_SHEET_PATH& aObservationSheet, double aTraceInductance,
                                double aTraceResistance )
{
    LOCALE_IO dummy;
    ...
    sim->Init();
    wxString netlist =
            BuildSpiceNetlist( aNetworkKey, aObservationSheet, aTraceInductance, aTraceResistance );
